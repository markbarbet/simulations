...
for i in nominal_models:
    ...
    newfilename=ntpath.dirname(i)+'\\modified_'+ntpath.basename(i)
...
def solver(conditions):
            ...
            for j in np.arange(len(T)):
                try:
                    try:
                        nominalDelays.append(ig.ignition_delay(nominal_model,T[j],P,X,options=reactorType,igdelay_type=igdelay_type))
                    except:
                        new_dict={}
                        new_dict[fuels]=X.pop(fuels)
                        new_dict['o2']=X.pop('O2')
                        new_dict['n2']=X.pop('N2')
                        nominalDelays.append(ig.ignition_delay(nominal_model,T[j],P,new_dict,options=reactorType,igdelay_type=igdelay_type))
                except:
                    nominalDelays.append('Temperature '+str(T[j])+'K failed')
            for j in np.arange(len(T)):
                try:
                    try:
                        modifiedDelays.append(ig.ignition_delay(modified_model,T[j],P,X,options=reactorType,igdelay_type=igdelay_type))
                    except:
                        new_dict={}
                        new_dict[fuels]=X.pop(fuels)
                        new_dict['o2']=X.pop('O2')
                        new_dict['n2']=X.pop('N2')
                        modifiedDelays.append(ig.ignition_delay(modified_model,T[j],P,new_dict,options=reactorType,igdelay_type=igdelay_type))
                except:
